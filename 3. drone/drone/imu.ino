...
void gyro_setup()
{
  ...
  // Check MPU6050 connection and wait until connected blink green_led1 every 0.5 seconds until mpu6050 connected
  led_output(0, 1, 0);
  while (true)
  {
    ...
    {
      Serial.println("Could not find a valid MPU6050 sensor, check wiring!"); // connection failed
      blinkLED(green1, 500);
    }
  }

  // wakes up the MPU6050 by setting the SLEEP bit to 0
  Wire.beginTransmission(0x68);
  Wire.write(0x6B);
  Wire.write(0x00);
  Wire.endTransmission();

  // to get the bias offset values of the mpu6050 using calibrate_gyro(2000) function
